    ...
    public partial class ReportCreator : Form
    {
        ...
        private void ReportCreator_Load(object sender, EventArgs e) {
            ...
            try {
                ...
                RefreshComboBox();
                DBConnect.FindReportSaveLocation();
        ...
        private void btnExport_Click(object sender, EventArgs e) {
            ...
            //else if () { }
                ...
                if (stuID == -1) {
                    ...
                    try {
                        DBConnect.InsertStudent(thisStudent);
                    }
                ...
                if (passStudent) {
                    //CREATE LESSON
                    passLesson = DBConnect.InsertLesson(new Lesson(
                        DBConnect.NewLessID(),
                        DBConnect.FindStuID(cbStudentName.Text),
                        DateTime.Parse(ConvertDateFormat(tDate.Text)),
                        tTopic.Text,
                        tHomework.Text));
                    
                    //CREATE REPORT
                    if (passLesson & chbReport.Enabled & TablesContainData()) {
        ...
        private bool TablesContainData() {
            foreach (var table in new DataGridView[] {
                dgvCorrections, 
                dgvNewLanguage, 
                dgvPronunciation}) 
            {
                for (int i = 0; i < table.Rows.Count; i++) {
                    string cell = (string)table.Rows[i].Cells[0].Value;
